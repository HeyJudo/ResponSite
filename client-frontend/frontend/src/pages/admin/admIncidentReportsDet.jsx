...
import { updateIncidentStatus, deleteIncident, assignRespondent, resolveIncidentWithNotes } from '../../API/incidentService';
...
const AdmIncidentReportsDet = () => {
  ...
  return (
    <div className="dashboard-root">
              ...
              <div style={{ padding: '0 1rem 1rem 1rem' }}>
                <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', marginTop: '0', gap: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', alignItems: 'stretch', width: '50%' }}>
                    {error && <div style={{ color: 'red', padding: '10px', background: '#ffe9e9', borderRadius: '5px', marginBottom: '10px' }}>{error}</div>}
                    <button onClick={handleMarkResolved} disabled={status === 'RESOLVED' || loading} style={{
                      background: status === 'RESOLVED' ? '#ccc' : '#001D9C',
                      color: status === 'RESOLVED' ? '#999' : '#fff',
                      border: 'none',
                      borderRadius: '5px',
      ...
      <Modal isOpen={showDeleteConfirmation} onClose={() => setShowDeleteConfirmation(false)} title="Delete Incident" showFooter={false}>
        <div style={{ padding: '1.5rem' }}>
          <p style={{ marginBottom: '1.5rem', fontSize: '1rem' }}>Are you sure you want to delete this incident? This action cannot be undone.</p>
          <div style={{ display: 'flex', gap: '0.8rem', justifyContent: 'flex-end' }}>
